...
public:
    ...
    {
        putDirect(exec->propertyNames().prototype, JSPositionErrorCallbackPrototype::self(exec), None);
    }
...
JSObject* JSPositionErrorCallbackPrototype::self(ExecState* exec)
{
    return getDOMPrototype<JSPositionErrorCallback>(exec);
}
...
JSObject* JSPositionErrorCallback::createPrototype(ExecState* exec)
{
    return new (exec) JSPositionErrorCallbackPrototype(JSPositionErrorCallbackPrototype::createStructure(exec->lexicalGlobalObject()->objectPrototype()));
}
...
JSValuePtr jsPositionErrorCallbackConstructor(ExecState* exec, const Identifier&, const PropertySlot& slot)
{
    return static_cast<JSPositionErrorCallback*>(asObject(slot.slotBase()))->getConstructor(exec);
}
JSValuePtr JSPositionErrorCallback::getConstructor(ExecState* exec)
{
    return getDOMConstructor<JSPositionErrorCallbackConstructor>(exec);
}

JSValuePtr jsPositionErrorCallbackPrototypeFunctionHandleEvent(ExecState* exec, JSObject*, JSValuePtr thisValue, const ArgList& args)
{
    UNUSED_PARAM(args);
    ...
    PositionError* error = toPositionError(args.at(exec, 0));

    imp->handleEvent(error);
    return jsUndefined();
}

JSC::JSValuePtr toJS(JSC::ExecState* exec, PositionErrorCallback* object)
{
    return getDOMObjectWrapper<JSPositionErrorCallback>(exec, object);
}
PositionErrorCallback* toPositionErrorCallback(JSC::JSValuePtr value)
{
