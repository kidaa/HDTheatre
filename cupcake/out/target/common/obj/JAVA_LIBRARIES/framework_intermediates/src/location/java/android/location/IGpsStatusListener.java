...
/**
 * Cast an IBinder object into an IGpsStatusListener interface,
...
mRemote.transact(Stub.TRANSACTION_onGpsStarted, _data, null, IBinder.FLAG_ONEWAY);
}
finally {
_data.recycle();
}
...
try {
_data.writeInterfaceToken(DESCRIPTOR);
mRemote.transact(Stub.TRANSACTION_onGpsStopped, _data, null, IBinder.FLAG_ONEWAY);
}
finally {
_data.recycle();
}
...
_data.writeInterfaceToken(DESCRIPTOR);
_data.writeInt(ttff);
mRemote.transact(Stub.TRANSACTION_onFirstFix, _data, null, IBinder.FLAG_ONEWAY);
}
finally {
_data.recycle();
}
...
mRemote.transact(Stub.TRANSACTION_onSvStatusChanged, _data, null, IBinder.FLAG_ONEWAY);
}
finally {
_data.recycle();
}
...
static final int TRANSACTION_onGpsStarted = (IBinder.FIRST_CALL_TRANSACTION + 0);
static final int TRANSACTION_onGpsStopped = (IBinder.FIRST_CALL_TRANSACTION + 1);
static final int TRANSACTION_onFirstFix = (IBinder.FIRST_CALL_TRANSACTION + 2);
static final int TRANSACTION_onSvStatusChanged = (IBinder.FIRST_CALL_TRANSACTION + 3);
}
public void onGpsStarted() throws android.os.RemoteException;
public void onGpsStopped() throws android.os.RemoteException;
public void onFirstFix(int ttff) throws android.os.RemoteException;
