...
mRemote.transact(Stub.TRANSACTION_onLocationChanged, _data, null, IBinder.FLAG_ONEWAY);
}
finally {
_data.recycle();
}
...
mRemote.transact(Stub.TRANSACTION_onStatusChanged, _data, null, IBinder.FLAG_ONEWAY);
}
finally {
_data.recycle();
}
...
_data.writeInterfaceToken(DESCRIPTOR);
_data.writeString(provider);
mRemote.transact(Stub.TRANSACTION_onProviderEnabled, _data, null, IBinder.FLAG_ONEWAY);
}
finally {
_data.recycle();
}
...
_data.writeInterfaceToken(DESCRIPTOR);
_data.writeString(provider);
mRemote.transact(Stub.TRANSACTION_onProviderDisabled, _data, null, IBinder.FLAG_ONEWAY);
}
finally {
_data.recycle();
}
...
static final int TRANSACTION_onLocationChanged = (IBinder.FIRST_CALL_TRANSACTION + 0);
static final int TRANSACTION_onStatusChanged = (IBinder.FIRST_CALL_TRANSACTION + 1);
static final int TRANSACTION_onProviderEnabled = (IBinder.FIRST_CALL_TRANSACTION + 2);
static final int TRANSACTION_onProviderDisabled = (IBinder.FIRST_CALL_TRANSACTION + 3);
}
public void onLocationChanged(android.location.Location location) throws android.os.RemoteException;
public void onStatusChanged(java.lang.String provider, int status, android.os.Bundle extras) throws android.os.RemoteException;
public void onProviderEnabled(java.lang.String provider) throws android.os.RemoteException;
