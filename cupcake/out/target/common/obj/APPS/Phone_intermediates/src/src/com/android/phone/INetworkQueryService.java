...
/**
 * Cast an IBinder object into an INetworkQueryService interface,
...
mRemote.transact(Stub.TRANSACTION_startNetworkQuery, _data, null, IBinder.FLAG_ONEWAY);
}
finally {
_data.recycle();
}
...
_data.writeInterfaceToken(DESCRIPTOR);
_data.writeStrongBinder((((cb!=null))?(cb.asBinder()):(null)));
mRemote.transact(Stub.TRANSACTION_stopNetworkQuery, _data, null, IBinder.FLAG_ONEWAY);
}
finally {
_data.recycle();
}
...
static final int TRANSACTION_startNetworkQuery = (IBinder.FIRST_CALL_TRANSACTION + 0);
static final int TRANSACTION_stopNetworkQuery = (IBinder.FIRST_CALL_TRANSACTION + 1);
}
/**
     * Starts a network query if it has not been started yet, and
     * request a callback through the INetworkQueryServiceCallback
